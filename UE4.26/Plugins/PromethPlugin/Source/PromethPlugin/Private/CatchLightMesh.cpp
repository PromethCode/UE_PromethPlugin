...
public:
	...
	void UpdateSection(TArray<FVector>& veterics, TArray<FVector>& normals, TArray<FVector2D>& UVs, TArray<int32>& triangles, UMaterialInterface* material = nullptr)
	{
		...
		FVector* PositionBufferData = (FVector*)RHILockVertexBuffer(VertexBuffers.PositionVertexBuffer.VertexBufferRHI, 0, veterics.Num() * sizeof(FVector), RLM_WriteOnly);
		if (!PositionBufferData)
			return;
		FMemory::Memcpy(PositionBufferData, veterics.GetData(), veterics.Num() * sizeof(FVector));
		RHIUnlockVertexBuffer(VertexBuffers.PositionVertexBuffer.VertexBufferRHI);

		//FVector* TangentBufferData = (FVector*)RHILockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.VertexBufferRHI, 0, normals.Num() * sizeof(FVector), RLM_WriteOnly);
		...
		int16* TangentBufferData = (int16*)RHILockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.VertexBufferRHI, 0, veterics.Num() * sizeof(TangentType), RLM_WriteOnly);
		...
		RHIUnlockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.VertexBufferRHI);

		
		FVector2D* CoordinateBufferData = (FVector2D*)RHILockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.VertexBufferRHI, 0, UVs.Num() * sizeof(FVector2D), RLM_WriteOnly);
		if (!CoordinateBufferData)
			return;
		FMemory::Memcpy(CoordinateBufferData, UVs.GetData(), UVs.Num() * sizeof(FVector2D));
		RHIUnlockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.VertexBufferRHI);

		int32* IndexBufferData = (int32*)RHILockIndexBuffer(IndexBuffer.IndexBufferRHI, 0, MaxTriangles * 3 * sizeof(int32), RLM_WriteOnly);
		if (!IndexBufferData)
			return;
	...
	bool UpdateSection(MeshParam* pReader, FRHICommandListImmediate& RHICmdList, UMaterialInterface* material = nullptr)
	{
		...
		{
			//拷贝 顶点缓存
			float* PositionBufferData = (float*)RHICmdList.LockVertexBuffer(VertexBuffers.PositionVertexBuffer.VertexBufferRHI, 0, NumVerts * sizeof(FVector), RLM_WriteOnly);
			if (!PositionBufferData)
				return false;
			float* pos = PositionBufferData;

			FMemory::Memcpy(pos, vertices, NumVerts * sizeof(FVector));
			RHICmdList.UnlockVertexBuffer(VertexBuffers.PositionVertexBuffer.VertexBufferRHI);
		}
		...
		{
			typedef TStaticMeshVertexTangentDatum<typename TStaticMeshVertexTangentTypeSelector<EStaticMeshVertexTangentBasisType::HighPrecision>::TangentTypeT> TangentType;
			int16* TangentBufferData = (int16*)RHICmdList.LockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.VertexBufferRHI, 0, NumVerts * sizeof(TangentType), RLM_WriteOnly);
			if (!TangentBufferData)
				return false;
			...
			RHICmdList.UnlockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TangentsVertexBuffer.VertexBufferRHI);
		}
		...
		if (uvslen == 0)
			return false;
		//FVector2D* CoordinateBufferData = (FVector2D*)RHICmdList.LockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.VertexBufferRHI, 0, (uvslen >> 1) * sizeof(FVector2D), RLM_WriteOnly);
		//if (!CoordinateBufferData)
		//	return false;
		//FMemory::Memcpy(CoordinateBufferData, uvs, (uvslen >> 1) * sizeof(FVector2D));
		//RHICmdList.UnlockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.VertexBufferRHI);
		if (uvs)
		{
			FVector2D* CoordinateBufferData = (FVector2D*)RHICmdList.LockVertexBuffer(VertexBuffers.StaticMeshVertexBuffer.TexCoordVertexBuffer.VertexBufferRHI, 0, uvslen * sizeof(float), RLM_WriteOnly);
			if (!CoordinateBufferData)
				return false;
			...
			// Indices
			int32* IndexBufferData = (int32*)RHICmdList.LockIndexBuffer(IndexBuffer.IndexBufferRHI, 0, MaxTriangles * 3 * sizeof(int32), RLM_WriteOnly);
			//FMemory::Memset(IndexBufferData, 0, MaxTriangles * 3 * sizeof(int32));
			if (!IndexBufferData)
				return false;
			if (triangleslen == 0)
