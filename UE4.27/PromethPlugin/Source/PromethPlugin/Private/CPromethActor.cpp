...
void ACPromethActor::initTexture(UTexture2D** ptexture, int width, int height, EPixelFormat format, FSlateTexture2DRHIRef** SlateTexture2DRHI)
{
	...
	if (bValid) {
		...
		UE_LOG(LogTemp, Warning, TEXT(" [ NewDebug ] Texture is Valid"));
	}
	if (!bValid || bChange) {
		// UTexture2DExternal
			...
			if(SlateTexture2DRHI){
				ENQUEUE_RENDER_COMMAND(InitMovieTexture)(
					[=](FRHICommandListImmediate& RHICmdList)
					{
						...
						(*SlateTexture2DRHI) = new FSlateTexture2DRHIRef(width, height, format, NULL, ETextureCreateFlags::TexCreate_RenderTargetable, true);
						(*SlateTexture2DRHI)->InitResource();
					});
			}
		}
		UE_LOG(LogTemp, Warning, TEXT(" [ NewDebug ] Texture ChangeSize"));
	}
	...
	if (bisValidData) {
		unsigned char* TextureYUYVData = (unsigned char*)PicData.nativeTex;
		{
			...
			FRHICommandListImmediate& CommandList = RHICmdList;
			{
				...
				CommandList.SetViewport(0, 0, 0.0f, this->VideoWidth, this->VideoHeight, 1.0f);
				...
				//case EMediaTextureSampleFormat::CharYUY2:
				{
					...
					ConvertShader->SetParameters(RHICmdList, InputTarget, FIntPoint(this->VideoWidth, this->VideoHeight), MediaShaders::YuvToSrgbDefault, MediaShaders::YUVOffset8bits, false);
				}
